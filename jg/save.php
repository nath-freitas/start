<?php
// Persistência do "Plano de ação". Grava FORA da pasta publicada,
// para que nenhum deploy do Git apague os dados do cliente.
header('Content-Type: application/json; charset=utf-8');
$dataFile = '/home/u346131448/dados/jg-plano.json';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $in = json_decode(file_get_contents('php://input'), true);
  $plano = (is_array($in) && isset($in['plano']) && is_array($in['plano'])) ? $in['plano'] : [];
  @mkdir(dirname($dataFile), 0755, true);
  $payload = json_encode(['plano' => $plano, 'saved_at' => date('c')],
